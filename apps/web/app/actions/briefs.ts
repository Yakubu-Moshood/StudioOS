...
export async function createBriefVersion(input: {
  ...
  if (!content) return { success: false, error: 'Brief content is required.' }
  ...
  const { data, error } = await supabase.rpc('create_brief_version', {
    ...
    brief_content: { body: content },
  })

  if (error || !data) {
    return { success: false, error: 'Failed to save Brief version.' }
  }
...
export async function decideBriefVersion(input: {
  ...
  if (error || !data) {
    return { success: false, error: 'Failed to record Brief decision.' }
  }
