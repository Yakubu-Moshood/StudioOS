...
-- Durable Job/Run records wrap provider execution. Research output is immutable and versioned.

CREATE OR REPLACE FUNCTION public.start_research_run(target_production_id uuid)
RETURNS public.runs
LANGUAGE plpgsql
SECURITY INVOKER
...
BEGIN
  ...
  IF approved_brief_version_id IS NULL THEN
    RAISE EXCEPTION 'An approved Brief version is required';
  END IF;
  ...
  IF research_task_id IS NULL THEN RAISE EXCEPTION 'Research task not found'; END IF;

  IF EXISTS (
    SELECT 1 FROM public.jobs j
    WHERE j.task_id = research_task_id AND j.status IN ('queued', 'running')
  ) THEN
    RAISE EXCEPTION 'Research is already running';
  END IF;

  INSERT INTO public.jobs (task_id, job_type, status, created_by)
  VALUES (research_task_id, 'generate_research', 'running', auth.uid())
  RETURNING * INTO created_job;

  SELECT COALESCE(MAX(r.attempt), 0) + 1 INTO next_attempt
  FROM public.runs r
  JOIN public.jobs j ON j.id = r.job_id
  WHERE j.task_id = research_task_id;
  ...
  VALUES (
    ...
    next_attempt,
    'running',
    jsonb_build_object('brief_version_id', approved_brief_version_id)
  )
  RETURNING * INTO created_run;

  UPDATE public.production_stages SET status = 'active' WHERE id = research_stage_id;
  UPDATE public.production_tasks SET status = 'active' WHERE id = research_task_id;
  ...
  VALUES (target_production_id, 'run.started', auth.uid(), 'run', created_run.id,
    jsonb_build_object('job_type', 'generate_research'));
...
DECLARE
  ...
  research_stage_id uuid;
  research_task_id uuid;
  research_artifact public.artifacts;
  created_version public.artifact_versions;
  next_version integer;
BEGIN
  SELECT j.id, ps.production_id, ps.id, pt.id
  INTO target_job_id, target_production_id, research_stage_id, research_task_id
  FROM public.runs r
  JOIN public.jobs j ON j.id = r.job_id
  JOIN public.production_tasks pt ON pt.id = j.task_id
  JOIN public.production_stages ps ON ps.id = pt.stage_id
  WHERE r.id = target_run_id AND r.status = 'running' AND j.job_type = 'generate_research';

  IF target_job_id IS NULL THEN RAISE EXCEPTION 'Active Research run not found'; END IF;

  INSERT INTO public.artifacts (production_id, artifact_type, title)
  VALUES (target_production_id, 'research', 'Research')
  ON CONFLICT (production_id, artifact_type) DO UPDATE SET title = EXCLUDED.title
  RETURNING * INTO research_artifact;

  PERFORM 1 FROM public.artifacts WHERE id = research_artifact.id FOR UPDATE;
  SELECT COALESCE(MAX(version_number), 0) + 1 INTO next_version
  FROM public.artifact_versions WHERE artifact_id = research_artifact.id;

  INSERT INTO public.artifact_versions (artifact_id, version_number, content, created_by, source_run_id)
  VALUES (research_artifact.id, next_version, research_content, auth.uid(), target_run_id)
  ...
  UPDATE public.production_stages SET status = 'awaiting_approval' WHERE id = research_stage_id;
  UPDATE public.production_tasks SET status = 'awaiting_approval' WHERE id = research_task_id;
  ...
  VALUES
    ...
    (target_production_id, 'artifact.version_created', auth.uid(), 'artifact_version', created_version.id,
      jsonb_build_object('artifact_type', 'research', 'version_number', created_version.version_number));

  RETURN created_version;
END;
$$;

CREATE OR REPLACE FUNCTION public.fail_research_run(target_run_id uuid, failure jsonb)
RETURNS void
LANGUAGE plpgsql SECURITY INVOKER SET search_path = public AS $$
DECLARE
  target_job_id uuid; target_production_id uuid; research_stage_id uuid; research_task_id uuid;
BEGIN
  SELECT j.id, ps.production_id, ps.id, pt.id
  INTO target_job_id, target_production_id, research_stage_id, research_task_id
  FROM public.runs r
  ...
  UPDATE public.production_tasks SET status = 'failed' WHERE id = research_task_id;
...
LANGUAGE plpgsql SECURITY INVOKER SET search_path = public AS $$
DECLARE
  created_approval public.approvals; target_production_id uuid; target_artifact_id uuid;
  target_version integer; latest_version integer; research_stage_id uuid; script_stage_id uuid;
  research_task_id uuid; script_task_id uuid;
BEGIN
  ...
  WHERE av.id = target_artifact_version_id AND a.artifact_type = 'research';
  IF target_production_id IS NULL THEN RAISE EXCEPTION 'Research version not found'; END IF;
  SELECT MAX(version_number) INTO latest_version FROM public.artifact_versions WHERE artifact_id = target_artifact_id;
  IF target_version <> latest_version THEN RAISE EXCEPTION 'Only latest Research version may be decided'; END IF;
  ...
  SELECT ps.id, pt.id INTO research_stage_id, research_task_id
  FROM public.production_stages ps JOIN public.production_tasks pt ON pt.stage_id = ps.id
  WHERE ps.production_id = target_production_id AND ps.stage_key = 'research' AND pt.task_key = 'generate_research';
  ...
  IF approval_decision = 'approved' THEN
    UPDATE public.production_stages SET status = 'completed' WHERE id = research_stage_id;
    UPDATE public.production_tasks SET status = 'completed' WHERE id = research_task_id;
  ...
  ELSE
    ...
    UPDATE public.production_stages SET status = 'pending' WHERE id = script_stage_id;
    UPDATE public.production_tasks SET status = 'pending' WHERE id = script_task_id;
