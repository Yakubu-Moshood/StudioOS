...
alter table public.artifacts add constraint artifacts_artifact_type_check check (
  artifact_type = any (array[
    ...
    'ai_generation_package','generated_video','post_production_master','delivery_package'
  ])
);

create or replace function public.start_delivery_run(target_production_id uuid)
returns public.runs
...
begin
  ...
  where a.production_id = target_production_id
    and a.artifact_type = 'post_production_master'
    and ap.decision = 'approved'
  order by av.version_number desc limit 1;

  if approved_master_version_id is null then raise exception 'An approved Post-production Master is required'; end if;
  ...
  where ps.production_id = target_production_id
    and ps.stage_key = 'delivery'
    and pt.task_key = 'complete_delivery';

  if target_task_id is null then raise exception 'Delivery task not found'; end if;
  if exists (select 1 from public.jobs where task_id = target_task_id and status in ('queued','running')) then
    raise exception 'Delivery is already running';
  end if;

  insert into public.jobs (task_id, job_type, status, created_by)
  values (target_task_id, 'delivery', 'running', auth.uid())
  ...
  values (created_job.id, next_attempt, 'running', jsonb_build_object('post_production_master_version_id', approved_master_version_id))
  returning * into created_run;

  update public.production_stages set status='active' where production_id=target_production_id and stage_key='delivery';
  ...
  values (target_production_id,'run.started',auth.uid(),'run',created_run.id,jsonb_build_object('job_type','delivery'));
  return created_run;
end;
$$;

create or replace function public.complete_delivery_run(
  target_run_id uuid,
  delivery_url text,
...
begin
  if char_length(trim(delivery_url)) = 0 then raise exception 'Delivery URL is required'; end if;
  ...
  where r.id=target_run_id and r.status='running' and j.job_type='delivery';

  if target_job_id is null then raise exception 'Active Delivery run not found'; end if;

  insert into public.artifacts (production_id,artifact_type,title)
  values (target_production_id,'delivery_package','Delivery Package')
  ...
  values (target_artifact.id,next_version,jsonb_build_object(
    'delivery_url',trim(delivery_url),
    'delivery_method',delivery_method,
    'recipient_name',recipient_name,
    'metadata',delivery_metadata
  ),auth.uid(),target_run_id)
  returning * into created_version;

  update public.runs set status='succeeded',provider=delivery_method,output=jsonb_build_object('artifact_version_id',created_version.id,'delivery_url',trim(delivery_url),'metadata',delivery_metadata),finished_at=now() where id=target_run_id;
...
create or replace function public.decide_delivery_package_version(
...
begin
  ...
  where av.id=target_artifact_version_id and a.artifact_type='delivery_package';
  if target_production_id is null then raise exception 'Delivery Package version not found'; end if;
  select max(version_number) into latest_version from public.artifact_versions where artifact_id=target_artifact_id;
  if target_version<>latest_version then raise exception 'Only latest Delivery Package version may be decided'; end if;
  ...
  select position into current_position from public.production_stages where production_id=target_production_id and stage_key='delivery';
  if approval_decision='approved' then
    update public.production_stages set status='completed' where production_id=target_production_id and stage_key='delivery';
    update public.production_tasks set status='completed' where stage_id in (select id from public.production_stages where production_id=target_production_id and stage_key='delivery');
    ...
    update public.production_stages set status='ready' where production_id=target_production_id and stage_key='delivery';
    update public.production_tasks set status='ready' where stage_id in (select id from public.production_stages where production_id=target_production_id and stage_key='delivery');
